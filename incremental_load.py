...
def folder_maker(filename):
    dbf_file_path = f'/Volumes/DATA/F01/{filename}.dbf'
    csv_file_path = f'/Volumes/MARAL/CSV/F01/{filename}.csv'
    metadata_file_path = f'/Volumes/MARAL/CSV/F01/META/{filename}.json'
...
def read_dbf(dbf_file_path):
    ...
    dbf_table = DBF(dbf_file_path, load=True, encoding='latin-1', ignore_missing_memofile=True)
...
def get_last_timestamp(metadata_file_path):
    ...
    if os.path.exists(metadata_file_path):
        with open(metadata_file_path, 'r') as f:
            ...
            return datetime.strptime(metadata['last_processed_timestamp'], '%Y-%m-%d %H:%M:%S')
    return datetime(1980, 1, 1)  # Default to a very old date



def process_and_update_csv(dbf_file_path, csv_file_path, metadata_file_path, timestamp_field):
    ...
    df[timestamp_field] = df[timestamp_field].apply(lambda x: x if isinstance(x, pd.Timestamp) else pd.NaT)
    ...
    if not new_records_df.empty:
        ...
        new_records_df.to_csv(csv_file_path, mode='a', sep=';', encoding='latin-1', index=False)
        
        # Update or create metadata file
        latest_timestamp = new_records_df[timestamp_field].max()
        metadata = {"last_processed_timestamp": latest_timestamp.strftime('%Y-%m-%d %H:%M:%S')}
        with open(metadata_file_path, 'w') as f:
            json.dump(metadata, f)


def incremental_load_process(filename, timestamp_field):

    """Chain all steps to perform incremental load from DBF to CSV."""

    dbf_file_path,csv_file_path,metadata_file_path = folder_maker(filename)
    # Read DBF file
    ...
    process_and_update_csv(dbf_file_path, csv_file_path, metadata_file_path, timestamp_field)
    print(f"CSV files and metadata updated.")


# files = ['V2AD1001','V2AD1004','V2AD1005','V2AD1009','V2AR1001',
#          'V2AR1002','V2AR1004','V2AR1005','V2AR1006','V2AR1007',
#          'V2LA1001','V2LA1003','V2LA1005','V2LA1008','V4AR1009',
#          'V4LA1009','V2AD1056','V2AD1096']

files = ['V2AR1001']

# files = ['V2AD1001','V2AD1056','V2AD1096']
...
for file in files:
    ...
    incremental_load_process(file,timestamp_field)
    end_time = time.time()
