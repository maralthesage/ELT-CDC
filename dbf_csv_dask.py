...
def read_dbf(file_path):
    ...
    try:
        table = DBF(
            file_path, load=True, encoding="cp850", ignore_missing_memofile=True
        )
...
def etl_flow():
    ...
    for LAND in lands:
        for FILE_NAME in files:
            dbf_file_path = rf"\\10.10.1.23\data\{LAND}\{FILE_NAME}.dbf"
            csv_file_path = rf"\\10.10.1.23\maral\CSV\{LAND}\{FILE_NAME}.csv"
            ...
            if os.path.exists(csv_file_path):
                ...
                if modification_date != today_date:
                    ...
                    if (
                        df is not None
                    ):  # Proceed only if the DBF file was successfully read
