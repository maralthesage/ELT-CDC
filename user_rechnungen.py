...
spark = (
    SparkSession.builder.appName("User Rechnung Processing")
...
V2AD1056_df_edited = V2AD1056_df.select(
    "VERWEIS",
    "HERKUNFT",
    "AUFTRAG_NR",
    "RECH_NR",
...
V2AD1156_df_edited = V2AD1156_df.select(
    "RECHNUNG", "ART_NR", "GROESSE", "FARBE", "BEZEICHNG", "PREIS", "MENGE"
)
V2AD1001_df_edited = V2AD1001_df.select(
    "NUMMER", "ANREDE", "VORNAME", "NAME", "QUELLE", "GEBURT", "ORT", "SYS_ANLAGE"
)

# Extract substring from the 'VERWEIS' column and clean up 'NUMMER'
V2AD1056_df_edited = V2AD1056_df_edited.withColumn(
    "NUMMER", substring("VERWEIS", 3, 10).cast(StringType())
).withColumn("NUMMER", expr("lpad(NUMMER, 10, '0')"))

# Clean up 'RECH_NR' column
V2AD1056_df_edited = V2AD1056_df_edited.withColumn(
    "RECH_NR", col("RECH_NR").cast(StringType())
).withColumn("RECH_NR", expr("lpad(RECH_NR, 12, '0')"))

# Ensure 'NUMMER' is consistent across all DataFrames
V2AD1001_df_edited = V2AD1001_df_edited.withColumn(
    "NUMMER", col("NUMMER").cast(StringType())
).withColumn("NUMMER", expr("lpad(NUMMER, 10, '0')"))

V2AD1156_df_edited = V2AD1156_df_edited.withColumn(
    "RECHNUNG", col("RECHNUNG").cast(StringType())
).withColumn("RECHNUNG", expr("lpad(RECHNUNG, 12, '0')"))

# Join the DataFrames
rechnung = V2AD1156_df_edited.join(
    V2AD1056_df_edited,
    V2AD1156_df_edited.RECHNUNG == V2AD1056_df_edited.RECH_NR,
    "inner",
)
user_rechnung = rechnung.join(V2AD1001_df_edited, "NUMMER", "inner")

# Final selection of columns
user_rechnung = user_rechnung.select(
    "VERWEIS",
    "AUFTRAG_NR",
    "NUMMER",
    "ANREDE",
...
user_rechnung = (
    user_rechnung.withColumn("MENGE", col("MENGE").cast(FloatType()))
...
user_rechnung = (
    user_rechnung.withColumn("SYS_ANLAGE", col("SYS_ANLAGE").cast("timestamp"))
...
user_rechnung = (
    ...
    .filter(
        ~col("BEZEICHNG").rlike(
            r"(?i)Beilage|Anleit|Versand|Etikett|Katalog|Expresskosten|Pfandberechnung|rezept"
        )
    )
)
...
for key, value in anrede_map.items():
    user_rechnung = user_rechnung.withColumn(
        "ANREDE", when(col("ANREDE") == key, value).otherwise(col("ANREDE"))
    )

for key, value in herkunft_map.items():
    user_rechnung = user_rechnung.withColumn(
        "HERKUNFT", when(col("HERKUNFT") == key, value).otherwise(col("HERKUNFT"))
    )
df_single_partition = user_rechnung.repartition(1)

user_rechnung.coalesce(1).write.csv(
    "user_rechnungen.csv", header=True, sep=";", mode="overwrite"
)
